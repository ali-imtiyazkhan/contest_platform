  ...
  contests: [
    {
      ...
      challenges: [
        {
          id: randomUUID(),
          title: "Fix a bug with a test",
          points: 50,
          live: true,
        },
        {
          id: randomUUID(),
          title: "Build a small CLI tool",
          points: 80,
          live: true,
        },
        {
          id: randomUUID(),
          title: "Refactor with benchmarks",
          points: 100,
          live: false,
        },
      ],
...
export function startChallenge(
  ...
  const ch = state.contests
    .flatMap((c) => c.challenges)
    .find((x) => x.id === challengeId && x.live);
  ...
  const lb = state.leaderboard.get(user.id) ?? {
    userId: user.id,
    email: user.email,
    points: 0,
  };
...
export function getLeaderboard(): LeaderboardEntry[] {
  return Array.from(getState().leaderboard.values()).sort(
